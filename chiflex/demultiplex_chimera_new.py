...
#f_unique = open(os.path.join(args.output, "%s.unique.bed" % args.name), "w", template=samfile)
#f_control = open(os.path.join(args.output, "%s.control.bed" % args.name), "w", template=samfile)

##open output bam files for chimeric reads
#f_chimera_unique = open(os.path.join(args.output, "%s.chimera_unique.bed" % args.name), "w")
#f_chimera_control = open(os.path.join(args.output, "%s.chimera_control.bed" % args.name), "w")
...
with open(os.path.join(args.output, "%s.unique.bed" % args.name), "w") as f_unique, open(os.path.join(args.output, "%s.control.bed" % args.name), "w") as f_control, open(os.path.join(args.output, "%s.chimera_unique.bed" % args.name), "w") as f_chimera_unique , open(os.path.join(args.output, "%s.chimera_control.bed" % args.name), "w") as f_chimera_control:
	...
	for c, arwlist in enumerate(generator_segments(args.path)):
		...
		if(chimera):
			if(chimera.control):
				counts[1]+=1;
				f_chimera_control.write('%s\n' % chimera.doublebed());
			else:
				counts[0]+=1;
				f_chimera_unique.write('%s\n' % chimera.doublebed());
		#if(single):
			#if(single.is_control):
				#counts[3]+=1;
				#f_control.write('%s\n' % single.to_interval);
			#else:
				#counts[2]+=1;
