...
class Maf(object):
	def __init__(self, header, alignment):
	...
	def __str__(self):
		s = "region ID:%s\nreference_specie:%s\nchromosome:%s\nstrand:%s\nstart:%s\nend:%d\nalignment\n" % (self.id, self.ref_specie, self.chr, self.strand, self.start, self.end)
...
def conservation_simple_mismatch(mm, match, seqs, pos):
	...
	variants = sequence.diverge_with_1mm(seq, include = True)	
	for k, seq in seqs.iteritems():
...
def conservation_perfect(seed, seqs, ref_specie):
	...
	if(pos>-1):
		r = conservation_simple_perfect(seed.match, seqs, pos)
