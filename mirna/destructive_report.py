...
def interval2score(interval):
	return float(interval.attrs['dscore']) + float(interval.attrs['cons_dscore']) 


interactions = [];
for count, interval in enumerate(BedTool(args.path)):
	
	energy, pattern, basepairing, pval = get_rnahybrid(interval.attrs['seq'], interval.attrs['mseq'], system = rhsys, extended=True);
